...
-- ERROR HANDLING OF UNKNOWN MEMBERS
--LOADING LOCATION TABLE WITH UNKNOWN VALUES
INSERT INTO DIM_LOCATION(DimLocationID,SourceLocationID, Address, City, PostalCode, State_Province, Country) 
VALUES (-1, -1, '<Unknown>', '<Unknown>', '<Unknown>', '<Unknown>', '<Unknown>')

--LOADING LOCATION TABLE FROM STAGE TABLES
...
-- ERROR HANDLING OF UNKNOWN MEMBERS
--LOADING CHANNEL TABLE WITH UNKNOWN VALUES
...
--LOADING CHANNEL TABLE FROM STAGE TABLES
...
CREATE OR REPLACE TABLE DIM_RESELLER(
    ...
    ,DimLocationID INT NOT NULL, CONSTRAINT FK_DimLocationID FOREIGN KEY (DimLocationID) REFERENCES Dim_Location(DimLocationID) --Foreign Key
...
-- ERROR HANDLING OF UNKNOWN MEMBERS
--LOADING RESELLER TABLE WITH UNKNOWN VALUES
INSERT INTO Dim_Reseller (DimResellerID, DimLocationID, SourceResellerID, ResellerName, ContactName, PhoneNumber, Email)
VALUES(-1, -1, -1, '<Unknown>', '<Unknown>', '<Unknown>', '<Unknown>')



--LOADING RESELLER TABLE FROM STAGE TABLES
INSERT INTO Dim_Reseller(DimLocationID, SourceResellerID, ResellerName, ContactName, PhoneNumber, Email)
SELECT LD.DimLocationID, R.ResellerID, R.ResellerName, R.Contact, R.PhoneNumber, R.EmailAddress 
FROM Stage_Reseller R 
INNER JOIN Dim_Location LD
ON R.Address = LD.Address
...
CREATE OR REPLACE TABLE DIM_CUSTOMER(
    ...
    ,CONSTRAINT FK_DimLocationID FOREIGN KEY (DimLocationID) REFERENCES Dim_Location(DimLocationID) --Foreign Key
...
-- ERROR HANDLING OF UNKNOWN MEMBERS
--LOADING CUSTOMER TABLE WITH UNKNOWN VALUES
...
--LOADING CUSTOMER TABLE FROM STAGE TABLE
INSERT INTO Dim_Customer(DimLocationID, SourceCustomerID, FullName, FirstName, LastName, Gender, EmailAddress, PhoneNumber)
SELECT LD.DimLocationID, C.CustomerID, CONCAT(C.FirstName,' ',C.LastName), C.FirstName, C.LastName, C.Gender, C.EmailAddress, C.PhoneNumber
FROM Dim_Location LD JOIN Stage_Customer C
ON LD.Address = C.Address
...
-- ERROR HANDLING OF UNKNOWN MEMBERS
--LOADING STORE TABLE WITH UNKNOWN VALUES
INSERT INTO Dim_Store(DimStoreID, DimLocationID, SourceStoreID, StoreNumber, StoreManager)
VALUES(-1, -1, -1, -1,'<Unknown>')



--LOADING STORE TABLE FROM STAGE TABLE
INSERT INTO Dim_Store(DimLocationID, SourceStoreID, StoreNumber, StoreManager)
SELECT LD.DimLocationID, S.StoreID, S.StoreNumber, S.StoreManager 
FROM Stage_Store S 
INNER JOIN Dim_Location LD
ON LD.Address = S.Address
...
CREATE OR REPLACE TABLE DIM_PRODUCT(
    ...
    ,ProductRetailPrice FLOAT(6) NOT NULL
    ,ProductWholesalePrice FLOAT(6) NOT NULL
    ,ProductCost FLOAT(6) NOT NULL
    ,ProductRetailProfit FLOAT(6) NOT NULL
    ,ProductWholesaleUnitProfit FLOAT(6) NOT NULL
    ,ProductProfitMarginUnitPercent FLOAT(6) NOT NULL
);

DROP TABLE DIM_PRODUCT

-- ERROR HANDLING OF UNKNOWN MEMBERS
--LOADING PRODUCT TABLE WITH UNKNOWN VALUES
INSERT INTO DIM_PRODUCT(DimProductID,SourceProductID, SourceProductTypeID, SourceProductCategoryID, ProductName, ProductType, ProductCategory, 
                        ProductRetailPrice, ProductWholesalePrice, ProductCost, ProductRetailProfit, ProductWholesaleUnitProfit, ProductProfitMarginUnitPercent) 
VALUES (-1, -1, -1, -1, '<Unknown>', '<Unknown>', '<Unknown>', -1, -1, -1, -1, -1, -1)

--LOADING PRODUCT TABLE FROM STAGE TABLES
INSERT INTO DIM_PRODUCT(SourceProductID, SourceProductTypeID, SourceProductCategoryID, ProductName, ProductType, ProductCategory, 
                        ProductRetailPrice, ProductWholesalePrice, ProductCost, ProductRetailProfit, ProductWholesaleUnitProfit, ProductProfitMarginUnitPercent) 
SELECT P.ProductID, P.ProductTypeID, PT.ProductCategoryID, P.Product, PT.ProductType, PC.ProductCategory, 
P.Price, P.WholesalePrice, P.Cost, (P.Price - P.Cost) as RetailProfit, (P.WholesalePrice - P.Cost) as WholesaleProfit, (RetailProfit*100/P.Price) as ProductMarginPercent
FROM Stage_Product P 
-- Join Product Type table
